<html>
    ...
    <title>تسجيل الدخول | المعلمين - لوحة التحكم</title>
...
{{--                            <img class="brand-logo" alt="msbah admin logo" src="{{ asset('dashboard/app-assets/images/logo/logo.png') }}">--}}
                            <h3 class="brand-text">مجموعة الفرقان لتعليم القرآن - مصر</h3>
                        </a>
                    </li>
                            ...
                            <div class="card border-grey border-lighten-3 m-0">
                                <div class="card-header border-0">
                                    <div class="card-title text-center">
                                        <img width="50%" alt="msbah admin logo" src="{{ asset('dashboard/assets/img/logo-eg.png') }}">
                                    </div>
                                    <h6 class="card-subtitle line-on-side text-muted text-center font-small-3 pt-2">
                                        <span>مجموعة الفرقان لتعليم القرآن - مصر || دخول المعلمين</span>
                                    </h6>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        <form class="form-horizontal" method="POST" action="{{ route('teachers.login.post') }}" novalidate>
                                            @csrf

{{--                                            <fieldset class="form-group position-relative has-icon-left">--}}
{{--                                                <input name="teacher_number" type="text"  class="form-control input-lg @error('teacher_number') is-invalid @enderror" id="user-name" placeholder="رقم المعلم"--}}
{{--                                                       tabindex="1" data-validation-required-message="يرجى إدخال رقم المعلم الصحيح." value="{{ old('teacher_number') }}" required autocomplete="teacher_number" autofocus>--}}
...
{{--                                            </fieldset>--}}
                                            <fieldset class="form-group position-relative has-icon-left">
                                                <input name="email" type="email"  class="form-control input-lg @error('email') is-invalid @enderror" id="user-name" placeholder="بريد المعلم"
                                                       tabindex="1" data-validation-required-message="يرجى إدخال بريد المعلم الصحيح." value="{{ old('email') }}" required autocomplete="email" autofocus>
                                                @error('email')
                                                    <span class="invalid-feedback" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
